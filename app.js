...
function mainDataReceived(data) {
    ...
    try {
        ...
        if (freshDatabase.length > 0) {
            ...
            try { createHaijinList(); } catch(e) { console.error('俳人リスト作成エラー:', e); }
        }
    } catch (error) {
        console.error('データ解析エラー:', error);
    } finally {
        // ★ 成功してもエラーが起きても確実にローディングを解除
        hideLoadingOverlay();
    }
}
...
/* 🌸 作品鑑賞画面の表示更新（おみ句じ季節では初期状態で伏せてiマークを表示） */
function updateHaikuDisplay() {
    ...
    // おみ句じ（全句・号内全句・季節）では初期状態を伏せ（iマーク表示）に設定
